...
//     if (lower.includes('broasc') || lower.includes('țestoas') || lower.includes('testoas')) return '🐢';
//     if (lower.includes('aric')) return '🦔';
//     if (lower.includes('pas') || lower.includes('păs') || lower.includes('papagal')) return '🦜';
//     if (lower.includes('iep')) return '🐰';
//     if (lower.includes('hamster') || lower.includes('porcușor') || lower.includes('porcusor') || lower.includes('roz')) return '🐹';
//     return '🐾';
//   };
...
//           {Object.keys(speciesStats).length > 0 && (
//             <View style={[styles.cardFull, styles.shadow, { paddingVertical: 15 }]}>
//               <Text style={[styles.cardLabel, { marginBottom: 10, marginLeft: 0 }]}>Cazări per specie pentru {formatDisplayDate(selectedDate)}</Text>
//               <View style={styles.speciesContainer}>
//                 {Object.entries(speciesStats).map(([spName, count], idx) => (
//                   <View key={idx} style={styles.speciesCircle}>
//                     <Text style={styles.speciesEmoji}>{getEmojiForSpecies(spName)}</Text>
...
//                 <Ionicons name="wallet-outline" size={20} color="#f59e0b" />
//                 <Text style={styles.cardLabel}>
//                   Venit {incomeDates.start ? `(${formatDisplayDate(incomeDates.start)} - ${formatDisplayDate(incomeDates.end)})` : "(Luna Curentă)"}
//                 </Text>
//               </View>
//               <Ionicons name="filter-outline" size={20} color="#cbd5e1" />
//             </View>
...
//                 <Text style={styles.bookingName}>{booking.pet_name} <Text style={{fontSize: 12, color: '#94a3b8', fontWeight: 'normal'}}>({booking.species_name})</Text></Text>
//                 <Text style={styles.bookingDates}>
//                   {formatDisplayDate(getLocalDateString(booking.start_date))} - {formatDisplayDate(getLocalDateString(booking.end_date))}
//                 </Text>
//               </View>
//               <View style={[
//                 styles.statusBadge, 
...
//                   setIncomeDates({ start: day.dateString, end: '' });
//                 } else {
//                   if (day.dateString > incomeDates.start) {
//                     setIncomeDates({ ...incomeDates, end: day.dateString });
//                   } else {
//                     setIncomeDates({ start: day.dateString, end: '' });
//                   }
...
export default function HotelDashboardScreen() { 
  ...
  const generateMarkedDates = () => {
    ...
    if (data?.bookingIntervals) {
      data.bookingIntervals.forEach(interval => {
        if (interval.status === 'anulat') return;
        let start = new Date(interval.start_date);
        let end = new Date(interval.end_date);
  ...
  const handleDeleteBooking = async () => {
    try {
      const res = await fetch(`http://172.20.10.2:3000/api/dashboard/booking/${activeBooking.id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        setActionModalVisible(false);
    ...
    <View style={styles.mainContainer}>
      ...
      >
        ...
        {dayBookings.length === 0 ? (
          ...
          dayBookings.map((booking, index) => (
            <View key={index} style={[styles.bookingItem, styles.shadow, booking.status === 'anulat' && { opacity: 0.6 }]}>
              <View style={styles.bookingIcon}>
                <Text style={{fontSize: 20}}>{getEmojiForSpecies(booking.species_name || '')}</Text>
              </View>
              <View style={styles.bookingInfo}>
                <Text style={[styles.bookingName, booking.status === 'anulat' && { textDecorationLine: 'line-through', color: '#94a3b8' }]}>
              ...
              <View style={{ alignItems: 'flex-end', justifyContent: 'center' }}>
                <View style={[
                  styles.statusBadge, 
                  { backgroundColor: booking.status === 'approved' ? '#d1fae5' : booking.status === 'anulat' ? '#fee2e2' : '#fef3c7' }
                ]}>
                  <Text style={[
                    styles.statusText,
                    { color: booking.status === 'approved' ? '#059669' : booking.status === 'anulat' ? '#ef4444' : '#d97706' }
                  ]}>{booking.status}</Text>
                </View>
              </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.actionModalContent}>
            ...
            {activeBooking?.status !== 'anulat' && (
              <TouchableOpacity style={styles.actionBtn} onPress={() => handleUpdateStatus('anulat')}>
                <Ionicons name="close-circle-outline" size={22} color="#f59e0b" />
                <Text style={styles.actionBtnText}>Anulează Cazarea</Text>
              </TouchableOpacity>
            )}
...
const styles = StyleSheet.create({
  ...
  actionModalTitle: { fontSize: 18, fontWeight: 'bold', color: '#1e293b' },
  actionPetName: { fontSize: 14, color: '#64748b', marginTop: 5 },
  actionBtn: { flexDirection: 'row', alignItems: 'center', padding: 15, backgroundColor: '#fef3c7', borderRadius: 12, marginBottom: 10 },
  actionBtnText: { color: '#d97706', fontSize: 16, fontWeight: '600', marginLeft: 10 },
  ...
  actionBtnCancel: { padding: 15, backgroundColor: '#f1f5f9', borderRadius: 12, alignItems: 'center', marginTop: 5 },
  actionBtnTextCancel: { color: '#475569', fontSize: 16, fontWeight: 'bold' }
});