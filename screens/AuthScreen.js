...
//     try {
//       const res = await fetch('http://172.20.10.2:3000/api/auth/forgot-password', {
//         method: 'POST',
...
//       Alert.alert(data.success ? 'Succes' : 'Eroare', data.message);
//     } catch (error) {
...
//             {resetLoading ? 'Se procesează...' : 'Ai uitat parola?'}
//           </Text>
//         </TouchableOpacity>
//       )}
...
export default function AuthScreen({ route, navigation }) {
  ...
  const handleVerifyCode = async () => {
    ...
    setResetLoading(true);
    try {
      const res = await fetch('http://172.20.10.2:3000/api/auth/verify-reset-code', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSetNewPassword = async () => {
    ...
    setResetLoading(true);
    try {
      const res = await fetch('http://172.20.10.2:3000/api/auth/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
