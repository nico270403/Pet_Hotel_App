...
//           {pets.length === 0 && <Text style={{ color: "#db0e4f", marginBottom: 12 }}>Nu ai adăugat niciun animal încă.</Text>}
//         </>
//       }
//       renderItem={({ item }) => (
//         <View style={styles.petCard}>
//           {item.image ? <Image source={{ uri: item.image }} style={styles.petAvatar} /> : 
//             <View style={[styles.petAvatar, { justifyContent: 'center', alignItems: 'center'}]}>
...
import * as ImagePicker from 'expo-image-picker';
import AsyncStorage from '@react-native-async-storage/async-storage'; // NOU: Importăm stocarea permanentă
import { AuthContext } from "../context/AuthContext"; // NOU: Importăm contextul de autentificare
import PetBackground from './PetBackground';

export default function PetProfileScreen({ navigation }) {
  const { user } = useContext(AuthContext); // NOU: Luăm utilizatorul logat ca să știm al cui e profilul
  ...
  // NOU: Încărcăm animalele salvate în telefon când se deschide ecranul sau când se schimbă user-ul
  useEffect(() => {
    const loadPets = async () => {
      if (user?.id) {
        try {
          const savedPets = await AsyncStorage.getItem(`pets_user_${user.id}`);
          if (savedPets) {
            setPets(JSON.parse(savedPets)); // Convertim textul înapoi în listă de obiecte
          } else {
            setPets([]); // Dacă nu are nimic salvat, pornim cu lista goală
          }
        } catch (error) {
          console.error("Eroare la încărcarea animalelor:", error);
        }
      }
    };
    loadPets();
  ...
  // NOU: Funcție ajutătoare care rescrie lista în memoria telefonului la orice modificare
  const savePetsToStorage = async (updatedPets) => {
    if (user?.id) {
      try {
        await AsyncStorage.setItem(`pets_user_${user.id}`, JSON.stringify(updatedPets));
      } catch (error) {
        console.error("Eroare la salvarea animalelor:", error);
      }
    }
  };
  ...
  const handleSavePet = () => {
    ...
    setPets(updatedPets);
    savePetsToStorage(updatedPets); // NOU: Salvăm permanent în telefon!
    ...
    Alert.alert("Confirmare", "Sigur vrei să ștergi acest animal?", [
      ...
      { 
        ...
        onPress: () => {
          ...
          setPets(updatedPets);
          savePetsToStorage(updatedPets); // NOU: Actualizăm stocarea după ștergere!
        } 
      }
    ]);
  };
  ...
  const handleEditPet = (pet) => {
    ...
    setPets(updatedPets);
    savePetsToStorage(updatedPets); // NOU: Actualizăm stocarea când scoatem animalul pentru editare!
  };
    ...
    <FlatList
      ...
      ListHeaderComponent={
        <>
          ...
          <Text style={styles.listTitle}>Animalele mele</Text>
          {pets.length === 0 && <Text style={{ color: "#ff0000", marginBottom: 12 }}>Nu ai adăugat niciun animal încă.</Text>}
        </>
      }
      renderItem={({ item }) => (
        <View style={styles.petCard}>
