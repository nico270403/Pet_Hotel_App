...
// --- Funcții ajutătoare pentru manipularea datelor ---
const parseD = (str) => {
...
// 1. RUTA PRINCIPALĂ: Crearea rezervării (folosită și de chat, și de formular)
router.post("/bookings", async (req, res) => {
  try {
    const {
    ...
    const hotelInfo = await pool.query(`SELECT name, capacity FROM hotels WHERE id = $1`, [hotel_id]);
    const hotelCapacity = hotelInfo.rows[0]?.capacity || 5;
    const hotelName = hotelInfo.rows[0]?.name || "Hotel necunoscut";
    ...
    if (!isRequestedAvailable) {
      ...
      // Căutăm sugestii în trecut (fără a depăși ziua de azi)
    ...
    // Verificare compatibilitate animal
    const allAnimalsResult = await pool.query(`SELECT id, name FROM animals`);
    const normalizedInput = pet_type.trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    ...
    // Inserare rezervare finală (adaptată la schema originală a bazei de date)
    const result = await pool.query(
      `INSERT INTO bookings (
        hotel_id, user_id, animal_id, pet_name, pet_type, start_date, end_date, 
        price_total, currency, status, owner_email, created_at
    ...
    try {
      ...
      await transporter.sendMail({
        from: `"TotoPet Hotel" <${process.env.EMAIL_USER}>`,
        to: process.env.EMAIL_ADMIN,
        subject: `Confirmă cererea lui ${owner_name} la hotel ${hotelName}`,
        html: `
          <h2>Rezervare nouă</h2>
      ...
      console.warn("Eroare la trimiterea emailului către admin:", emailErr.message);
    }
...
// 2. RUTA: Aprobare rezervare (Apelată din emailul adminului)
router.get("/:id/accept", async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
...
// 3. RUTA: Respingere rezervare (Apelată din emailul adminului)
router.get("/:id/reject", async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
...
// 4. RUTA: Calendar Frontend (Afișează datele indisponibile cu roșu în ReservationScreen)
router.get("/unavailable-dates/:hotelId", async (req, res) => {
  try {
    const { hotelId } = req.params;
    ...
    // Aici se selectează doar acele date unde numărul de animale este egal sau mai mare decât capacitatea hotelului
    const fullDates = Object.keys(occupancy).filter(date => occupancy[date] >= capacity);
